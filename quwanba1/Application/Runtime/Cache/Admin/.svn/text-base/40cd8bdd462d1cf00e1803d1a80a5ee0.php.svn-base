		...
		<div class="panel-body">
			...
			<div class="search-form fr cf pull-right">
				<div class="sleft form-inline">
					<a class=" btn btn-primary" href="<?php echo U('Ad/ad_add_page');?>"><i class="btn-search"></i>添加广告</a>
				</div>
			</div>
		    <!-- 数据列表 -->
		    ...
		    <tbody>
				<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php echo ($vo["discribe"]); ?></td>
					<td>
						<?php switch($vo["position"]): case "1": ?>商城首页顶部轮播<?php break;?>
					...
					<td>
						<a href="<?php echo U('Ad/ad_detail',array('id'=>$vo['id']));?>">查看</a>
						<a href="<?php echo U('Ad/ad_edit_page',array('id'=>$vo['id']));?>">修改</a>
...
<script type="text/javascript">
	...
	<script>
		function classDel(id){
			if (confirm("你确定删除吗？")) {
				$.ajax({
					url: "<?php echo U('Ad/ad_del');?>",
					type: 'post',
					data: {'id': id},
					success: function (data) {
						window.location.href = "<?php echo U('Ad/index');?>";
					}
				});
			}
		}
